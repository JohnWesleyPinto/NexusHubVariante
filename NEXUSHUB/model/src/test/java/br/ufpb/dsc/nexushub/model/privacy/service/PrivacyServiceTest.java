...
class PrivacyServiceTest{
 ...
 @Test void consentRequestAndExport(){UUID id=UUID.randomUUID();User user=mock(User.class);Human human=mock(Human.class);Role role=mock(Role.class);when(users.findById(id)).thenReturn(Optional.of(user));when(user.getId()).thenReturn(id);when(user.getHuman()).thenReturn(human);when(human.getName()).thenReturn("Ana");when(human.getUsername()).thenReturn("ana_dev");when(human.getCourse()).thenReturn("Ciencia da Computacao");when(user.getEmail()).thenReturn("a@b.com");when(user.getRole()).thenReturn(role);when(role.getName()).thenReturn("STUDENT");when(consents.save(any())).thenAnswer(i->i.getArgument(0));when(requests.save(any())).thenAnswer(i->i.getArgument(0));assertTrue(service.consent(id,"analytics","1",true).isGranted());assertEquals("EXPORT",service.request(id,"EXPORT").getType());Map<String,Object> export=service.export(id);assertEquals("Ana",export.get("name"));assertNotNull(export.get("profile"));}
